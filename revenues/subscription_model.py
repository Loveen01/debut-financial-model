...
class SubscriptionModel(BaseModel):
    ...
    def __init__(self, tier_settings, newly_acquired_customers):
        self.tier_settings = tier_settings # tuple representing (percentage_of_customers, price)
        self.newly_acquired_customers = newly_acquired_customers # dataFrame with 2 rows, 1 for new cus, and other cumulative
        
        self.no_tiers = len(self.tier_settings)

    def __init_empty_df(index_no, row_names = None):
        '''Creates an empty dataframe, filed with zeros'''
        if row_names is None:
            row_names = [f'Row_{i}' for i in range(index_no)]
        elif len(row_names) != index_no and index_no!=None:
    ...
    def __apply_churn_to_tier_acquired_customers(self, tier_level):
        ...
        cumulative_customers_after_churn = self.__init_empty_dict(1)
        cumulative_customers_after_churn.iloc[0,0] = monthly_customers.iloc[0,0] # assign first item as first item in cumulative 

        for i in range(1,13):
            cumulative_customers_after_churn.iloc[0,i] = (cumulative_customers_after_churn.iloc[0,i-1]*churn_rate
                                                          + monthly_customers[0,i] )
    ...
    def __apply_proportion_to_newly_acquired_customers(self, tier_level):
        proportion = self.__unwrap_tier_proportion(self, tier_level)
    ...
    def calc_total_revenue(self):
        ...
        for tier in range(self.no_tiers): 
            churned_newly_acquired_customers = self.__apply_churn_to_tier_acquired_customers(self, tier)
